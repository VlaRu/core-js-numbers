...
function isPowerOfTwo(num) {
  if (num <= 0 || num !== parseInt(num, 10) || Math.log2(num) % 1 !== 0) {
    return false;
  }
  return true;
}
...
function toPrecision(number, precision) {
  if (Number.isNaN(number) || Number.isNaN(precision) || precision < 1) {
    return false;
  }
